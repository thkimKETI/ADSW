...
def get_train_loader(cfg):
    if cfg.dataset == 'CULane':
        ...
        train_loader = TrainCollect(cfg.batch_size, 4, cfg.data_root, os.path.join(cfg.data_root, 'train', 'train_gt_depth.txt'), get_rank(), get_world_size(), 
                                cfg.row_anchor, cfg.col_anchor, cfg.train_width, cfg.train_height, cfg.num_cell_row, cfg.num_cell_col, cfg.dataset, cfg.crop_ratio) ## train_gt.txt
    else:
...
def inference_curvelanes(net, data_label):
    pred = net(data_label['images'], data_label['depth_images'])   ##
    cls_out_ext_label = (data_label['labels_row'] != -1).long()
    cls_out_col_ext_label = (data_label['labels_col'] != -1).long()
