import torch
from model.backbone_depth import resnet
import numpy as np
from utils.common_depth import initialize_weights
...
class parsingNet(torch.nn.Module):
    def __init__(self, pretrained=True, backbone='50', num_grid_row = None, num_cls_row = None, num_grid_col = None, num_cls_col = None, 
        ...
        self.cls_distribute = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.Conv2d(128, 8, 3, padding=1),   #Conv2d(128, 20, 3, ...
        )
    ...
    def forward(self, x, y):    ## (self, x)
        x2,x3,fea = self.model(x, y)    ## (self, x)
        if self.use_aux:
            seg_out = self.seg_head(x2, x3,fea)
        lane_token = self.cls_distribute(fea).reshape(-1, 8, 1, self.input_height//32, self.input_width//32)  #reshape(-1,20, 1 ...
        fea = self.pool(fea).unsqueeze(1).repeat(1, 8, 1, 1, 1) #repeat(1, 20, 1 ...
        fea = torch.cat([fea, lane_token], 2)

        fea = fea.view(-1, self.input_dim)
        out = self.cls(fea).reshape(-1, 8, 2048) # reshape(-1, 20, ...
        out_row = self.cls_row(out[:, :4, :]).permute(0, 2, 1)  #out[:, :10, ...
        out_col = self.cls_col(out[:, 4:, :]).permute(0, 2, 1) #out[:, 10:, ...
        ...
        pred_dict = {'loc_row': out_row[:,:self.dim1, :].reshape(-1,self.num_grid_row, self.num_cls_row, self.num_lane_on_row), 
                ...
                'lane_token_row': lane_token[:, :4, :, :].sum(1), 'lane_token_col': lane_token[:, 4:, :, :].sum(1)} # lane_token[:, :10, ... && lane_token[:, 10:
        if self.use_aux:
