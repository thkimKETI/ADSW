...
def get_curvelanes_list(root, label_dir):
    ...
    for img_name in tqdm.tqdm(os.listdir(os.path.join(l, 'images'))):
        ...
        label = img_name.replace('png', 'lines.json')
...
def generate_segmentation_and_train_list(root, line_txt, names, file_name='train_gt_depth.txt', json_name='curvelanes_anno_cache.json'):
    """
    ...
    for i in tqdm.tqdm(range(len(line_txt))):
        ...
        k_neg.sort()
        k_neg = k_neg[::-1] # add reverse
        k_pos.sort()
        label_path = 'segs'+names[i][6:-3]+'png'
        label = np.zeros((height,width),dtype=np.uint8)  
        bin_label = [0] * 4 # * 10

        all_points = np.zeros((4,125,2), dtype=np.float) # 10, 125
        the_anno_row_anchor = np.array(list(range(200, 1450, 10))) 
        all_points[:,:,1] = np.tile(the_anno_row_anchor, (4,1)) #(10,1)
        all_points[:,:,0] = -99999

        rw = 2560 / width
        rh = 1440 / height
        for idx in range(len(k_neg))[:2]: # [:5]
            which_lane = np.where(ks == k_neg[idx])[0][0]
            draw(label,lines[which_lane],2-idx) # [5-idx]
            bin_label[1-idx] = 1 # [4-idx]
            all_points[1-idx] = spline(np.array(lines[which_lane]), the_anno_row_anchor, ratio_height = rh, ratio_width = rw)
        
        for idx in range(len(k_pos))[:2]: # [:5]
            which_lane = np.where(ks == k_pos[idx])[0][0]  # -(idx+1)
            draw(label,lines[which_lane],3+idx) # [6+idx]
            bin_label[2+idx] = 1 # [5+idx]
            all_points[2+idx] = spline(np.array(lines[which_lane]), the_anno_row_anchor, ratio_height = rh, ratio_width = rw)
        ...
        train_gt_fp.write('train/'+names[i] + ' ' + 'train/' +label_path + ' '+ 'train/depth/'+ names[i][7:-4]+ '_disp.jpeg' + ' ' +' '.join(list(map(str,bin_label))) + '\n')
    train_gt_fp.close()
