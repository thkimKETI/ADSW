...
class LaneExternalIterator(object):
    ...
    def _prepare_train_batch(self):
        ...
        for _ in range(self.batch_size):
            ...
            img_path = os.path.join(self.path, depth_name)  ##
            with open(img_path, 'rb') as f: ##
                depth_images.append(np.frombuffer(f.read(), dtype=np.uint8))    ##
        ...
        return (images, seg_images, labels, depth_images)   ## (images, seg_images, labels)
    ...
    def _prepare_test_batch(self):
        ...
        for _ in range(self.batch_size):
            ...
            depth_name = self.list[self.i % self.n].split()[1]  ##
            ...
            img_path = os.path.join(self.path, img_name)
            depth_path = os.path.join(self.path, depth_name)  ##
        ...
        return images, names, depth_images  ## images, names
...
def ExternalSourceTrainPipeline(batch_size, num_threads, device_id, external_data, train_width, train_height, top_crop, normalize_image_scale = False, nscale_w = None, nscale_h = None):
    ...
    with pipe:
        jpegs, seg_images, labels, depth_images = fn.external_source(source=external_data, num_outputs=4) ## num_outputs=3
        images = fn.decoders.image(jpegs, device="mixed")
        seg_images = fn.decoders.image(seg_images, device="mixed")
        depth_images = fn.decoders.image(depth_images, device="mixed")  ##
        if normalize_image_scale:
            ...
            depth_images = fn.resize(depth_images, resize_x=nscale_w, resize_y=nscale_h, interp_type=types.INTERP_NN)   ##
        ...
        depth_images = fn.warp_affine(depth_images, matrix = mt, fill_value=0, inverse_map=False) ##
        ...
        depth_images = fn.resize(depth_images, resize_x=train_width, resize_y=int(train_height/top_crop), interp_type=types.INTERP_NN)  ##
        ...
        seg_images = fn.crop_mirror_normalize(seg_images, 
                                            ...
                                            std = [1., 1., 1.],
                                            crop = (train_height, train_width), crop_pos_x = 0., crop_pos_y = 1.)
        depth_images = fn.crop_mirror_normalize(depth_images, 
                                            dtype=types.FLOAT, 
                                            mean = [0., 0., 0.],
                                            std = [1., 1., 1.],
                                            crop = (train_height, train_width), crop_pos_x = 0., crop_pos_y = 1.)   ##
        pipe.set_outputs(images, seg_images, labels, depth_images)  ## (images, seg_images, labels)
    return pipe
...
def ExternalSourceTestPipeline(batch_size, num_threads, device_id, external_data):
    pipe = Pipeline(batch_size, num_threads, device_id)
    with pipe:
        jpegs, names, depth_images = fn.external_source(source=external_data, num_outputs=3)  ## num_outputs=2
        images = fn.decoders.image(jpegs, device="mixed")
        depth_images = fn.decoders.image(depth_images, device="mixed")  ##

        images = fn.resize(images, resize_x=800, resize_y=288)
        depth_images = fn.resize(depth_images, resize_x=800, resize_y=288)  ##
        images = fn.crop_mirror_normalize(images, 
                                            dtype=types.FLOAT, 
                                            mean = [0.485 * 255, 0.456 * 255, 0.406 * 255],
                                            std = [0.229 * 255, 0.224 * 255, 0.225 * 255])
        ...
        pipe.set_outputs(images, names, depth_images) ## (images, names)
    return pipe
...
class TrainCollect:
    ...
    dataset_name, top_crop):
        ...
        self.pii = DALIGenericIterator(pipe, output_map = ['images', 'seg_images', 'points', 'depth_images'], last_batch_padded=True, last_batch_policy=LastBatchPolicy.PARTIAL)    ##
    ...
    def __next__(self):
        ...
        depth_images = data[0]['depth_images']  ##
        ...
        return {'images':images, 'seg_images':seg_images, 'depth_images':depth_images, 'labels_row':labels_row, 'labels_col':labels_col, 'labels_row_float':labels_row_float, 'labels_col_float':labels_col_float}  ##
...
class TestCollect:
    def __init__(self, batch_size, num_threads, data_root, list_path, shard_id, num_shards):
        ...
        self.pii = DALIGenericIterator(pipe, output_map = ['images', 'names', 'depth_images'], last_batch_padded=True, last_batch_policy=LastBatchPolicy.PARTIAL)   ##
        self.eii_n = eii.n
    def __iter__(self):
        return self
    ...
    def __next__(self):
        ...
        out_dict = {'images': images, 'names': restored_names, 'depth_images': depth_images}    ##
        return out_dict
