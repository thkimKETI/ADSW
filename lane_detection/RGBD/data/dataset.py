...
class LaneTestDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        ##
        img_name = self.list[index].split()[0]
        depth_name = 'valid/depth'+ img_name[12:-4] + "_disp.jpeg"
        # depth_name = self.list[index].split()[1]  ##
        img_path = os.path.join(self.path, img_name)
        # print(img_path)
        depth_path = os.path.join(self.path, depth_name)  ##
        # print(depth_path)
        img = loader_func(img_path)
        depth = loader_func(depth_path) ##
        ##
