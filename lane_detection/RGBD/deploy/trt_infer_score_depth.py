...
color = [(255,0,0), (255,0,255), (0,255,0), (0,0,255)] # blue, magenta, green, red
...
class UFLDv2:
    ...
    def fusion_forward(self, img, dimg, gt_path):
        #for RGB image
        im0 = img.copy()
        img = img[self.cut_height:, :, :]
        img = cv2.resize(img, (self.input_width, self.input_height), cv2.INTER_CUBIC)
        ...
        # for Depth image
        dim0 = dimg.copy()
        dimg = dimg[self.cut_height:, :, :]
        dimg = cv2.resize(dimg, (self.input_width, self.input_height), cv2.INTER_CUBIC)
        dimg = dimg.astype(np.float32) / 255.0
        dimg = np.transpose(np.float32(dimg[:, :, :, np.newaxis]), (3, 2, 0, 1))
        dimg = np.ascontiguousarray(dimg)
        cuda.memcpy_htod(self.inputs[1]['allocation'], dimg)
        ...
        for lane in coords:
            if len(lane) != 0:
            	for idx, coord in lane[0]:
            	    cv2.circle(im0, coord, 2, color[idx], -1)
        cv2.imshow("result", im0)
...
def get_depth_path(path):
    ...
    depth_path = depth_path[:-4] + "_disp.jpeg"
    return depth_path
...
if __name__ == "__main__":
    ...
    if args.video_path is not None:
        ...
        for stop, path in enumerate(images):
            if stop == 10000:
                break
            print(path)
            ...
            dimg = dimg[144:,:,:]
            dimg = cv2.resize(dimg, (1600, 800))
            
            tp, fp, fn = isnet.fusion_forward(img, dimg, gt_path)
            TP += tp
            FP += fp
            FN += fn
