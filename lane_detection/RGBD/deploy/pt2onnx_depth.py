...
def get_args():
    ...
    parser.add_argument('--size', default=(1600, 800), help='size of original frame', type=tuple) # (1600,320)
...
def convert(model, args):
    ...
    depth_images = torch.zeros((1, 3, args.size[1], args.size[0])).cuda()
    onnx_path = args.model_path[:-4] + ".onnx"
    with torch.no_grad():
        torch.onnx.export(model, (images, depth_images), # images
                          onnx_path,
                          verbose=False,
                          input_names=['images', 'depth_images'], # ['input']
