...
def get_train_loader(batch_size, data_root, griding_num, dataset, use_aux, distributed, num_lanes):
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, sampler = sampler, num_workers=0) # num_workers=4->0
...
def get_test_loader(batch_size, data_root,dataset, distributed, crop_ratio, train_width, train_height):
    ...
    if dataset == 'CULane':
        ...
        test_dataset = LaneTestDataset(data_root,os.path.join(data_root, 'valid/valid_for_culane_style_depth.txt'),img_transform = img_transforms, crop_size=train_height)    ##
    else:
        raise NotImplementedError
    if distributed:
    ...
    loader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, sampler = sampler, num_workers=0) # num_workers=4->0
    return loader
